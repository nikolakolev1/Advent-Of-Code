...
/**
 * I found this on Git and thought that it is extremely useful. :)
 *
 * @author Michael Graat
 */
...
//    }
    ...
    private static void downloadInput(int day) throws IOException {
        ...
        if (!inputExists(day)) {
            ...
            System.out.println("The file '" + "input.txt"// getInputFilename(day)
                    + "' already exists in the data/day" + getDay() + " directory, so it will not be downloaded. Please remove it to download it again."); // 'resources' directory, so it will not be downloaded. Please remove it to download it again.");
        }
    }
    ...
    private static void writeInputToFile(int day, List<String> input) throws IOException {
        String filename = "input.txt"; // getInputFilename(day);
        Path path = Paths.get("data", "day" + getDay(), filename); // ("resources", filename);
        Files.createFile(path);
    ...
    private static boolean inputExists(int day) {
        String filename = "input.txt"; // getInputFilename(day);
        Path p = Paths.get("data", "day" + getDay(), filename); // ("resources", filename);
...
//    }
    ...
    private static List<String> getInput(int day, String sessionId, String emailAddress) throws IOException {
        ...
        try {
            ...
            con.setRequestProperty("User-Agent", "github.com/nikolakolev1/Advent-Of-Code by " + emailAddress);
